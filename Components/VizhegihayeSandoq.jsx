export default function VizhegihayeSandoq() {
  return (
    <div className="mx-auto max-w-5xl px-6 py-20 sm:px-8 lg:py-24" id="features">
      <div className="pb-12 text-center">
        <h2 className="pb-4 text-2xl font-bold text-slate-900 md:text-3xl">
          ویژگی‌های صندوق
        </h2>
        <p className="text-slate-900"></p>
      </div>
      <div className="mx-auto grid max-w-5xl grid-cols-1 gap-8 text-center sm:grid-cols-2 sm:gap-0 md:text-right">
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6 ">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 576 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6 "
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M279.9 17.4c5.2-1.9 10.9-1.9 16.1 0L407.2 57 288 102.7 168.8 57 279.9 17.4zM144 218.5l-127.7 43C6.6 264.8 0 274 0 284.3V434.5c0 13.2 8.1 25 20.3 29.8l112.2 43.9c7.3 2.9 15.4 2.9 22.8 .2l132.7-49 132.7 49c7.4 2.7 15.5 2.6 22.8-.2l112.2-43.9c12.3-4.8 20.3-16.6 20.3-29.8V284.3c0-10.3-6.6-19.5-16.3-22.7L432 218.5V65.8c0-10.1-6.4-19.2-15.9-22.6L301.4 2.3c-8.7-3.1-18.2-3.1-26.9 0L159.9 43.2C150.4 46.6 144 55.7 144 65.8V218.5zm16 .4V70.8l120 46v152L160 218.9zm136 72.9l128 49.8v151L296 445.3V291.8zm-144.3-59l114.1 47.4L144.2 327.5 24.8 275.6l126.9-42.8zM16 434.5V289.2l120 52.2v151l-109.8-43C20 447 16 441.1 16 434.5zm136 58.1v-151l128-49.8V445.3L152 492.6zM296 116.8l120-46V218.9L296 268.8v-152zm14.2 163.4l114.1-47.4 126.9 42.8L431.8 327.5 310.2 280.2zM549.8 449.4L440 492.4v-151l120-52.2V434.5c0 6.6-4 12.5-10.2 14.9z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                تنوع سرمایه‌گذاری
              </h3>
              <p className="text-slate-900">
                شما می‌توانید با توجه به میزان ریسک پذیری خود در صندوق‌های مختلف
                سرمایه‌گذاری کنید.
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 576 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M310.5 144h-109l-5.3 3.4C141.4 182.5 32 268.3 32 416c0 35.3 28.7 64 64 64H416c35.3 0 64-28.7 64-64c0-147.7-109.4-233.5-164.3-268.6l-5.3-3.4zm-3.8-41.8L355.1 32H156.9l48.4 70.2 6.7 9.8h87.8l6.7-9.8zM0 416C0 273.4 91.2 183.7 152.6 138.5c9.7-7.1 18.6-13.2 26.4-18.1L160.8 94 121.9 37.6C111 21.7 122.4 0 141.7 0H370.3c19.3 0 30.7 21.7 19.8 37.6L351.2 94 333 120.4c7.7 4.9 16.7 11 26.4 18.1C420.8 183.7 512 273.4 512 416c0 53-43 96-96 96H96c-53 0-96-43-96-96z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                مدیریت سرمایه
              </h3>
              <p className="text-slate-900">
                کارشناسان ما با بررسی همیشگی، سرمایه شما را مدیریت می‌کنند تا
                بهترین تجربه سرمایه‌گذاری را داشته باشید.
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 576 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M117.2 136C160.3 96 217.6 64 288 64s127.7 32 170.8 72c43.1 40 71.9 88 85.2 120c-13.3 32-42.1 80-85.2 120c-43.1 40-100.4 72-170.8 72s-127.7-32-170.8-72C74.1 336 45.3 288 32 256c13.3-32 42.1-80 85.2-120zM288 32c-80.8 0-145.5 36.8-192.6 80.6C48.6 156 17.3 208 2.5 243.7c-3.3 7.9-3.3 16.7 0 24.6C17.3 304 48.6 356 95.4 399.4C142.5 443.2 207.2 480 288 480s145.5-36.8 192.6-80.6c46.8-43.5 78.1-95.4 93-131.1c3.3-7.9 3.3-16.7 0-24.6c-14.9-35.7-46.2-87.7-93-131.1C433.5 68.8 368.8 32 288 32zM192 256a96 96 0 1 1 192 0 96 96 0 1 1 -192 0zm224 0a128 128 0 1 0 -256 0 128 128 0 1 0 256 0z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                نظارت بر بازار
              </h3>
              <p className="text-slate-900">
                کارشناسان ما با نظارت و بررسی همیشگی بازار، سرمایه‌گذاری در
                مناسب ترین دارایی‌ها را انجام می‌دهند.
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 512 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M256 336c68.5 0 129.7-15.6 173.2-40.1c44-24.8 66.8-56.5 66.8-87.9s-22.8-63.1-66.8-87.9C385.7 95.6 324.5 80 256 80s-129.7 15.6-173.2 40.1C38.8 144.9 16 176.6 16 208s22.8 63.1 66.8 87.9C126.3 320.4 187.5 336 256 336zM0 208C0 128.5 114.6 64 256 64s256 64.5 256 144l0 16 0 96c0 70.7-114.6 128-256 128S0 390.7 0 320l0-96 0-16zM168 343.3l0 80.7c24.8 4.7 51.8 7.5 80 8l0-80c-28-.5-54.9-3.5-80-8.7zm-16-3.6c-23.4-5.9-44.9-13.6-64-23L88 399c18.6 8.7 40.2 16 64 21.6l0-81zM16 258.2L16 320c0 24 18.2 49.5 56 70.8l0-82.7C47.7 294 28.4 277 16 258.2zM344 424l0-80.7c-25.1 5.2-52 8.2-80 8.7l0 80c28.2-.4 55.2-3.2 80-8zm16-84.3l0 81c23.8-5.5 45.4-12.8 64-21.6l0-82.4c-19.1 9.3-40.6 17.1-64 23zM496 320l0-61.8c-12.4 18.8-31.7 35.7-56 49.9l0 82.7c37.8-21.3 56-46.8 56-70.8zM256 128c-49.9 0-94.8 9.3-126.8 23.9C96.5 166.8 80 185.9 80 204s16.5 37.2 49.2 52.1c32 14.6 76.9 23.9 126.8 23.9s94.8-9.3 126.8-23.9c32.7-14.9 49.2-34 49.2-52.1s-16.5-37.2-49.2-52.1C350.8 137.3 305.9 128 256 128zm192 76c0 28.2-24.7 51.2-58.6 66.7C354.9 286.5 307.7 296 256 296s-98.9-9.5-133.4-25.3C88.7 255.2 64 232.2 64 204s24.7-51.2 58.6-66.7C157.1 121.5 204.3 112 256 112s98.9 9.5 133.4 25.3C423.3 152.8 448 175.8 448 204z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                نقد شوندگی لحظه‌ای
              </h3>
              <p className="text-slate-900">
                صندوق‌های سرمایه‌گذاری نقد شوندگی بسیار بالایی دارند و شما در هر
                لحظه می‌توانید دارایی خود را نقد کنید.
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 384 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M96 32C78.3 32 64 46.3 64 64V448c0 17.7 14.3 32 32 32H288c17.7 0 32-14.3 32-32V64c0-17.7-14.3-32-32-32H96zM32 64C32 28.7 60.7 0 96 0H288c35.3 0 64 28.7 64 64V448c0 35.3-28.7 64-64 64H96c-35.3 0-64-28.7-64-64V64zM160 400h64c8.8 0 16 7.2 16 16s-7.2 16-16 16H160c-8.8 0-16-7.2-16-16s7.2-16 16-16z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                سادگی در استفاده
              </h3>
              <p className="text-slate-900">
                با استفاده از اپلیکیشن سرمایه‌گذاری اهرم، سرمایه‌گذاری خود را به
                سادگی شروع کنید.
              </p>
            </div>
          </div>
        </div>
        <div className="space-y-4 p-4 hover:cursor-default sm:p-6">
          <div className="flex flex-col items-center justify-center gap-4 gap-y-6 md:flex-row md:items-start md:justify-start md:gap-6">
            <div className="rounded-2xl bg-gray-100 p-4 text-slate-900  ">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                viewBox="0 0 640 512"
                fill="fill-black"
                stroke="currentColor"
                strokeWidth="2"
                strokeLinecap="round"
                strokeLinejoin="round"
                className="h-6 w-6"
              >
                <path
                  xmlns="http://www.w3.org/2000/svg"
                  d="M263.9 64c-33.9 0-66.7 12-92.6 33.7L144 120.7v39.1c2.7-.5 5.3-1.7 7.4-3.5l40.5-34c13.3-11.2 28.9-19 45.5-23.1L272.6 64h-8.7zM112 352.5c8.1 1.3 15.7 5 21.6 10.8l46.1 44.6 24.3 24.3c25.1 25.1 65.4 26.2 91.8 2.6c.8-.7 1.6-1.4 2.3-2.2c26.3 21.3 65.3 18.6 88.4-7.3c6-6.7 10.3-14.4 13-22.5c19.2 4 40-.8 55.7-14.8c11.3-10.1 18.2-23.2 20.6-37H528V368c0 26.5 21.5 48 48 48h16c26.5 0 48-21.5 48-48V160c0-17.7-14.3-32-32-32H560 528v32V318.8H473.1c-3.1-8.7-8.1-17-15.1-24l-85.6-85.6 8.1-7.5c6.5-6 6.9-16.1 .9-22.6s-16.1-6.9-22.6-.9l-76.1 70.2c-13.2 12.2-33.6 12.4-47 .4c-15.4-13.7-15.7-37.5-.8-51.6l79.4-75C332.2 105.4 355.8 96 380.4 96c22.7 0 44.6 8 61.9 22.6l44.8 37.7c2.5 2.1 5.6 3.4 8.8 3.7V121.8L462.9 94.1C439.8 74.6 410.6 64 380.4 64c-32.8 0-64.3 12.5-88.1 35l-79.4 75c-28.6 27-27.9 72.7 1.5 98.8c25.8 22.9 64.7 22.5 90-.8l44.5-41 86.5 86.5c13 13 12.4 34.3-1.4 46.5c-10.9 9.7-26.7 10.6-38.6 3c-5-3.3-11.4-3.4-16.6-.5s-8.3 8.6-8 14.6c.4 8.1-2.3 16.3-8.1 22.9c-12.3 13.9-33.8 14.5-46.9 1.4l-8.1-8.1c-3.6-3.6-8.7-5.3-13.7-4.5s-9.4 3.9-11.8 8.4c-1.9 3.6-4.4 6.9-7.6 9.8c-13.8 12.3-34.8 11.7-47.8-1.3l-39.9-39.8-.3 .3-30.7-29.7c-11.9-11.5-27.4-18.5-43.8-20V160 128H80 32c-17.7 0-32 14.3-32 32V368c0 26.5 21.5 48 48 48H64c26.5 0 48-21.5 48-48V352.5zM80 160V368c0 8.8-7.2 16-16 16H48c-8.8 0-16-7.2-16-16V160H80zm528 0V368c0 8.8-7.2 16-16 16H576c-8.8 0-16-7.2-16-16V160h48z"
                ></path>
              </svg>
            </div>
            <div>
              <h3 className="pb-2 text-xl font-semibold text-slate-900">
                مشاوره سرمایه‌گذاری
              </h3>
              <p className="text-slate-900">
                در تمام مراحل سرمایه‌گذاری کنار شما هستیم و شما را همراهی
                می‌کنیم.
              </p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
