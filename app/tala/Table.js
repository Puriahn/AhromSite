          ...
          {chart.map((col) => (
            <tr>
              ...
              <td className="whitespace-nowrap border border-r-0 border-slate-200 px-3 py-3 dark:border-navy-500 lg:px-5">
                <p
                  dir="ltr"
                  className={`font-medium font-16 text-right ${parseFloat(data.tala[col.percent])<0 ? 'text-rose-500' : 'text-green-800'} `}
                >
                  {data.tala[col.percent] === null
                    ? "-"
                    : hazfManfi(
                        toFarsiNumber(
                          parseFloat(data.tala[col.percent]).toFixed(2)
                        )
                      )}
                  {data.tala[col.percent] !== null&&'%'}
                </p>
